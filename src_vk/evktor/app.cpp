...
void App::setupBasePass() {
    ...
    {
        // static auto spv_pbr = Assets::loadSpirv("pbr.slang.spv");
        // static auto spv = ShaderModuleState().setCode(spv_pbr).into(vkt).unwrap();
        // .addVertShader(spv, "vertMain")
        // .addFragShader(spv, "fragMain")
    ...
    {
        // tex_cube = newBox<TextureCube>(VK_FORMAT_R8G8B8A8_SRGB, VkExtent2D(w, h), Texture2D::Transfer | Texture2D::Sampled,
        // 1);
        for (auto& face : {Assets::tex("spacebox1/right.png"),
...
void App::drawGui(const core::CommandBuffer& cmdbuf, float delta_time) {
