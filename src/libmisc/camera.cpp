...
void CameraArcball::processCameraMove(Movement dir, float delta_time) {
    float vel = delta_time * this->speed;
    switch (dir) {
        case Reset: {
            // 回复初始视角
...
#else
    // BUG eye旋转有问题
    glm::vec3 p0 = this->arcball_coordinate(pa);
    glm::vec3 p1 = this->arcball_coordinate(pb);

    // da, dr均为eye0=(0, 0, 1)相对center0=(0, 0, 0)的旋转
    float da = glm::acos(glm::dot(p0, p1)) * this->sensitivity;
    if (da > 0.0f) {
...
glm::vec3 CameraArcball::arcball_coordinate(const glm::vec2& p) {
...
void CameraFirstPerson::processMouseMove(const glm::vec2& pa, const glm::vec2& pb) {
    glm::vec2 d = (pb - pa) * this->sensitivity;
    pitch += d.y;
    yaw += d.x;
