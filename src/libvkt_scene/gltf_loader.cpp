...
Box<Sampler> GLTFLoader::parseSampler(const tinygltf::Sampler& gsampler) const {
    auto core_sampler = core::SamplerState()
                            .setFilter(getMagFilter(gsampler.magFilter), getMinFilter(gsampler.minFilter))
                            .setAddressMode(getWrapMode(gsampler.wrapS), getWrapMode(gsampler.wrapT))
                            .into(api)
                            .unwrap();
    core_sampler.setDebugName(gsampler.name);
...
Box<Scene> GLTFLoader::loadScene(int32_t scene_index) {
    ...
    for (size_t k = 0; k < gmodel.samplers.size(); k++) {
        samplers[k] = parseSampler(gmodel.samplers[k]);
    }
    ...
    auto cmdpool = core::CommandPoolState("CommandPool.Scene")
                       .setFlags(VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT)
                       .setQueueFamilyIndex(queue.family_index)
                       .into(api)
                       .unwrap();
    auto& cmdbuf = cmdpool.allocate(core::CommandPool::Level::Primary, "CommandBuffer.Scene").unwrap().get();
    ...
    for (size_t k = 0; k < gmodel.images.size(); k++) {
        ...
        scene->textures.push_back(Texture2D::from(api,
                                                  ...
                                                  vkt::Texture::Sampled | vkt::Texture::Transfer,
                                                  mip_levels)
                                      .unwrap());
        auto staging = core::BufferState()
