...
void testCoreApi() {
    ...
    CoreApi api_;
    {
        DebugState dso{};
        dso.setMessageSeverity(false, false);
        auto& instance = api_.borrow(api.inst()).unwrap().get();
        auto& idebug = api_.add(dso).unwrap().get();
        auto& debug = *dynamic_cast<const Debug*>(&idebug);
        auto& phy_dev = api_.borrow(api.phyDev()).unwrap().get();
        auto& device = api_.borrow(api.dev(), api.queueFamilyIndices()).unwrap().get();
